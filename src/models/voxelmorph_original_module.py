...
class VoxelmorphOriginalModule(BaseModule_Registration):
    ...
    def model_step(self, batch: Any, is_3d=False, is_train=False):
        ...
        if is_3d:
            original_slices = moving_img.shape[-1]
            moving_img = self.pad_slice_to_128(moving_img)
            fixed_img = self.pad_slice_to_128(fixed_img)
        ...
        if is_train:
            return moving_img, fixed_img, warped_img, deform_field
        else:
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        if self.params.flag_train_fixed_moving:
            moving_img, fixed_img, warped_img = self.model_step_for_swap_moving_fixed(batch,
                                                                                                      is_3d=self.params.is_3d)
        else:
            moving_img, fixed_img, warped_img, deform_field = self.model_step(batch,
                                                                                              is_3d=self.params.is_3d,
                                                                                              is_train=True)
