...
class BaseModule_Registration(LightningModule):  # single direction
    def __init__(self, params, *args: Any, **kwargs: Any):
        ...
        self.val_gc_B, self.val_nmi_B, self.val_sharpness_B, self.val_l2_B, self.val_fid_B = self.define_metrics()
        self.test_gc_B, self.test_nmi_B, self.test_sharpness_B, self.test_l2_B, self.test_fid_B = self.define_metrics()

        self.val_metrics = [self.val_gc_B, self.val_nmi_B, self.val_sharpness_B, self.val_l2_B, self.val_fid_B]
        self.test_metrics = [self.test_gc_B, self.test_nmi_B,  self.test_sharpness_B, self.test_l2_B, self.test_fid_B]

        self.nmi_scores = []

        self.test_csv_filename = 'test_metrics_ref_ct_cbct.csv'
        self.init_csv()

    def init_csv(self):
        """Initialize the CSV file and add the header."""
        if not os.path.exists(self.test_csv_filename):
            with open(self.test_csv_filename, mode='w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([
                    'batch_idx', 'gc_B', 'nmi_B', 'sharpness_B', 'l2_B'
                ])
    @staticmethod
    def define_metrics():
        ...
        return gc, nmi, sharpness, l2, fid
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        start = time.time()
        moving_img, fixed_img, warped_img = self.model_step(batch, is_3d=self.params.is_3d) # MR, CT, syn_CT, _

        if len(fixed_img.size()) == 5: # B x C x H x W x D (3D image)
            for i in range(fixed_img.size(4)):  # slice dim
                self.val_gc_B.update(norm_to_uint8(fixed_img[:, :, :, :, i]), norm_to_uint8(warped_img[:, :, :, :, i]))
                nmi_score = self.val_nmi_B(flatten_to_1d(norm_to_uint8(fixed_img[:, :, :, :, i])), flatten_to_1d(norm_to_uint8(warped_img[:, :, :, :, i])))
                ...
                self.val_nmi_B.reset()
        elif len(fixed_img.size()) == 4: # 2D image)
            self.val_gc_B.update(norm_to_uint8(fixed_img), norm_to_uint8(warped_img))
            nmi_score = self.val_nmi_B(flatten_to_1d(norm_to_uint8(fixed_img)), flatten_to_1d(norm_to_uint8(warped_img)))
            self.nmi_scores.append(nmi_score)
            ...
            ValueError(f"Unexpected number of dimensions in Image: {len(fixed_img.size())}. Expected 4 or 5.")
        end = time.time()
        # print("val sample took", end - start)
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        moving_img, fixed_img, warped_img = self.model_step(batch, is_3d=self.params.is_3d) # MR, CT, syn_CT, _
        # end = time.time()
        # print(end-start, "ELAPSED TIME FOR ONE SAMPLE")
        if len(fixed_img.size()) == 5:
            for i in range(fixed_img.size(4)):  # slice dim
                self.test_gc_B.update(norm_to_uint8(fixed_img[:, :, :, :, i]), norm_to_uint8(warped_img[:, :, :, :, i]))
                nmi_score = self.test_nmi_B(flatten_to_1d(norm_to_uint8(fixed_img[:, :, :, :, i])), flatten_to_1d(norm_to_uint8(warped_img[:, :, :, :, i])))
                self.nmi_scores.append(nmi_score)
                ...
                # self.test_kid_B.update(gray2rgb(norm_to_uint8(moving_img[:, :, :, :, i])), reafixed_img[:, :, i, :, :]l=True)
                ...
                self.test_nmi_B.reset()

        elif len(fixed_img.size()) == 4: # 4D tensor
            self.test_gc_B.update(norm_to_uint8(fixed_img), norm_to_uint8(moving_img))
            nmi_score = self.test_nmi_B(flatten_to_1d(norm_to_uint8(fixed_img)), flatten_to_1d(norm_to_uint8(moving_img)))
            self.nmi_scores.append(nmi_score)
            self.test_fid_B.update(gray2rgb(norm_to_uint8(moving_img)), real=True)
            self.test_fid_B.update(gray2rgb(norm_to_uint8(fixed_img)), real=False)
            ...
            raise ValueError(f"Unexpected number of dimensions in evaluation_img: {len(fixed_img.size())}. Expected 4 or 5.")

        gc_B = self.test_gc_B.compute()
        sharpness_B = self.test_sharpness_B.compute()
        l2_B = self.test_l2_B.compute()

        # Write the metrics to the CSV file
        with open(self.test_csv_filename, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([
                batch_idx, gc_B.item(), nmi_score.item(), sharpness_B.item(), l2_B.item()
            ])
