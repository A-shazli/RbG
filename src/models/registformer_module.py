...
class RegistFormerModule(BaseModule_AtoB):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        # self.criterionMSE = torch.nn.MSELoss() if params.lambda_mse != 0 else None
        # self.criterionGRAD = Grad(penalty="l2") if params.lambda_smooth != 0 else None
        #
        # self.criterionNMI = NormalizedMutualInfoScore() if params.lambda_nmi != 0 else None
        # self.metricnmi = NormalizedMutualInfoScore()
    ...
    def backward_G(self, real_a, real_b, fake_b):
        ...
        # if self.criterionMSE:
        #     nmi = self.metricnmi(flatten_to_1d(norm_to_uint8(real_b)), flatten_to_1d(norm_to_uint8(fake_b)))
        #     self.log("NMI", nmi.detach(), prog_bar=True)
        #     loss_mse = self.criterionMSE(real_b, fake_b) * self.params.lambda_mse
        #     self.log("MSE_Loss", loss_mse.detach(), prog_bar=True)
        #     loss_G += loss_mse
        #
        # if self.criterionGRAD:
        #     loss_smooth = self.criterionGRAD(fake_b) * self.params.lambda_smooth
        #     self.log("SMOOTH_Loss", loss_smooth.detach(), prog_bar=True)
        #     loss_G += loss_smooth
        #
        # if self.criterionNMI:
        #     nmi = self.criterionNMI(flatten_to_1d(norm_to_uint8(real_b)), flatten_to_1d(norm_to_uint8(fake_b)))
        #     self.log("NMI", nmi.detach(), prog_bar=True)
        #     nmiloss = 1 - nmi.detach()
        #     self.log("NMI_loss", nmiloss, prog_bar=True)
        #     loss_G += nmiloss
