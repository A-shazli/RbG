...
class GradICON(nn.Module):
    def __init__(self, **kwargs):
        ...
        # )
        phi = FunctionFromVectorField(tallUNet2(dimension=self.dimension))
        ...
        if self.dimension == 3:
            ...
            input_shape = [BATCH_SIZE, 1, self.input_size[1] // SCALE, self.input_size[2] // SCALE]

        # self.fullres_net.assign_identity_map([BATCH_SIZE, 1, 128, 128, 128])
...
class GradientICON(RegistrationModule):
    ...
    def compute_similarity_measure(self, phi_AB, phi_BA, image_A, image_B):
        ...
        # print(self.warped_image_A.shape, "Shape of the warped image A")
        # print(self.warped_image_B.shape, "Shape of the warped image B")
        # print(image_A.shape, "Shape of the image A")
        # print(image_B.shape, "Shape of the image B")
    ...
    def forward(self, image_A, image_B) -> ICONLoss:
        ...
        #     image_A, size=self.identity_map.shape[2:], mode="bilinear", align_corners=False
        # )
        # image_B = F.interpolate(
        #     image_B, size=self.identity_map.shape[2:], mode="bilinear", align_corners=False
        # )
        # print("Image A reshaped shape:", image_A.shape)
        ...
        # Must be set at beginning of forward b/c not preserved by .cuda() etc
        self.identity_map.isIdentity = True
        # returen phi_BA and warp using it make sure to warp image B to train reverse
        self.phi_AB = self.regis_net(image_A, image_B)
        self.phi_BA = self.regis_net(image_B, image_A)
...
class MINDSSC(SimilarityBase):
    def __init__(self, radius=1, dilation=1):
        self.radius = radius
    ...
    def pdist_squared(self, x):
        ...
        dist[dist != dist] = 0  # Handle NaN
        dist = torch.clamp(dist, 0.0, torch.inf)
        return dist

    def compute_mindssc(self, img):
        # Define start and end locations for self-similarity pattern in 2D
        four_neighbourhood = torch.Tensor([
            [0, 1],  # Right
            [1, 0],  # Down
            [0, -1],  # Left
            [-1, 0],  # Up
        ...
        # Compute patch-SSD for 2D
        rpad1 = torch.nn.ReplicationPad2d(self.dilation)
        rpad2 = torch.nn.ReplicationPad2d(self.radius)

        ssd_list = []
        for shift in four_neighbourhood:
            shifted_img = torch.roll(img, shifts=(shift[0], shift[1]), dims=(-2, -1))
            ssd = F.avg_pool2d(rpad2((rpad1(img) - rpad1(shifted_img)) ** 2), kernel_size, stride=1)
            ssd_list.append(ssd)

        # Stack SSDs and normalize
        ssd = torch.stack(ssd_list, dim=1)
    ...
    def __call__(self, image_A, image_B):
        ...
        assert len(image_A.shape) - 2 == 2, "The input image should be 2D."
...
class MIND_loss(torch.nn.Module):
    ...
    def MINDSSC(self, img, radius=2, dilation=2):
        ...
        # define start and end locations for self-similarity pattern (2D version)
        ...
        mshift1 = torch.zeros(6, 1, 3, 3).cuda()  # Change to 2D kernel
        mshift1.view(-1)[torch.arange(6) * 9 + idx_shift1[:, 0] * 3 + idx_shift1[:, 1]] = 1
        mshift2 = torch.zeros(6, 1, 3, 3).cuda()  # Change to 2D kernel
        mshift2.view(-1)[torch.arange(6) * 9 + idx_shift2[:, 0] * 3 + idx_shift2[:, 1]] = 1
        rpad1 = nn.ReplicationPad2d(dilation)  # Change to 2D padding
        rpad2 = nn.ReplicationPad2d(radius)   # Change to 2D padding
        ...
        mind = mind[:, torch.Tensor([4, 5, 0, 3, 1, 2]).long(), :, :]
