...
class ImageLoggingCallback(Callback):
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if len(batch[0].size()) == 5: # 3D Image
            self.val_batch_idx = [0, 1, 6, 12, 13]
        ...
        if (
            ...
            if len(batch[0].size()) == 5: # 3D Image
                ...
                moving_img, fixed_img, warped_img = pl_module.model_step(batch, is_3d=True)

                moving_img = moving_img[:, :, :, :, d_index].squeeze(-1)
                fixed_img = fixed_img[:, :, :, :, d_index].squeeze(-1)
                warped_img = warped_img[:, :, :, :, d_index].squeeze(-1)
                self.saving_to_grid([moving_img, fixed_img, warped_img])
    ...
    def on_test_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        if (
            ...
            if len(batch[0].size()) == 5: # 3D Image
                d_index = 30  # 5번째 D 차원의 30번째 데이터 선택
                moving_img, fixed_img, warped_img = pl_module.model_step(batch)
...
class ImageSavingCallback(Callback):
    ...
    def save_nii_registration(a_nii, b_nii, c_nii, d_nii, subject_number, folder_path):
        nib.save(a_nii, os.path.join(folder_path, f"moving_img_{subject_number}.nii.gz"))
        nib.save(b_nii, os.path.join(folder_path, f"fixed_img_{subject_number}.nii.gz"))
        nib.save(c_nii, os.path.join(folder_path, f"warped_img_{subject_number}.nii.gz"))
        if d_nii is not None:
            nib.save(
                d_nii, os.path.join(folder_path, f"black{subject_number}.nii.gz")
            )
        return
    ...
    def saving_to_nii(self, a, b, preds_a, preds_b=None):
        ...
        if a.ndim == 2: # 2D image
            ...
            if len(self.img_a) == self.subject_slice_num[0]:
                a_nii = np.stack(self.img_a, 0)
                b_nii = np.stack(self.img_b, 0)
                preds_a_nii = np.stack(self.img_preds_a, 0)
                if preds_b is not None:
                    preds_b_nii = np.stack(self.img_preds_b, 0)
                else:
                    preds_b_nii = a_nii * 0  # Placeholder if preds_b is None

                # matplotlib.pyplot.imshow(preds_a_nii[:, :, 100])
    ...
    def on_test_start(self, trainer, pl_module):
        ...
        with h5py.File(data_path, "r") as file:
            ...
            self.subject_slice_num = [
                mr_group[key].shape[2] for key in self.dataset_list # changed to 0
